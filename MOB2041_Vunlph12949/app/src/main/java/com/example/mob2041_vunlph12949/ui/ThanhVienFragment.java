                ...
                if (edHoten.getText().length() == 0 || edNamSinh.getText().length() == 0) {
                    ...
                    if (checkYear(edNamSinh)) {
                        ...
                        if (edHoten.getText().toString().length() < 5 || edHoten.getText().toString().length() > 15) {
                            Toast.makeText(getContext(), "Tên người dùng có độ dài tối thiểu 5, tối đa 15", Toast.LENGTH_SHORT).show();
                        } else if (!edHoten.getText().toString().substring(0, 1).toUpperCase().equals(edHoten.getText().toString().substring(0, 1))) {
...
////                manager.beginTransaction().replace(R.id.fragmentThanhVien,new SeachThanhVienFragment, manager.findFragmentByTag("fragmentThanhVien"));
////                break;
//            case R.id.menuAdd:
//                break;
//        }
